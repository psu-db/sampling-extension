...
public:
    SamplingFramework(size_t buffer_cap, size_t scale_factor, double max_tombstone_prop, double max_rejection_rate) :
    ...
    void range_sample(record_t *sample_set, const skey_t& lower_key, const skey_t& upper_key, size_t sample_sz, gsl_rng *rng) {
        psudb::Alias *buffer_alias = nullptr;
        std::vector<record_t *> buffer_records;
        size_t buffer_cutoff = 0;

        double buffer_weight;

        if (REJECTION_SAMPLING) {
            buffer_weight = m_buffer->get_total_weight(); 
            buffer_cutoff = m_buffer->get_record_count() - 1;
        } else {
            buffer_weight = m_buffer->get_sample_range(lower_key, upper_key, &buffer_alias, buffer_records, &buffer_cutoff);
        }

        // Get the shard weights for each level. Index 0 is the buffer,
        // represented by nullptr.
        std::vector<std::pair<ShardId, ShardWIRS *>> shards;
        std::vector<ShardWIRSState*> states;
        ...
        do {
            ...
            while (shard_samples[0] > 0) {
                const record_t *rec;
                if (REJECTION_SAMPLING) {
                    rec = m_buffer->get_sample(lower_key, upper_key, rng);
                } else {
                ...
                if (REJECTION_SAMPLING && buffer_rejections >= sample_sz && sample_idx == 0 && shard_weights.size() == 1) {
                    if (buffer_alias) delete buffer_alias;
                    for (auto& x: states) delete x;
                    return; // no records in the sampling range
                }
            ...
            for (size_t i=1; i<shard_samples.size(); i++) {
                ...
                auto sampled = shards[i].second->get_samples(states[i], sample_set + sample_idx, lower_key, upper_key, shard_samples[i], &state, rng) ;
    ...
    bool is_deleted(const record_t *record, const ShardId &shid, size_t buffer_cutoff) {
        ...
        auto res = m_levels[shid.level_idx]->check_tombstone(shid.shard_idx + 1, *record);
    ...
    ShardWIRS *create_static_structure() {
        std::vector<ShardWIRS *> shards;
        ...
        shards.emplace_back(new ShardWIRS(m_buffer, nullptr, DELETE_POLICY));

        ShardWIRS *shards_array[shards.size()];
        ...
        ShardWIRS *flattened = new ShardWIRS(shards_array, j, nullptr, DELETE_POLICY);
...
bool check_deleted(const record_t* record, sample_state *state) {
    return state->framework->is_deleted(record, state->shid, state->buffer_cutoff);
}

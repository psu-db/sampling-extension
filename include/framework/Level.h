...
private:
    struct InternalLevelStructure {
        ...
        : m_cap(cap)
        , m_shards(new ShardWIRS*[cap]{nullptr})
...
public:
    ...
    static Level* merge_levels(Level* base_level, Level* new_level, bool tagging) {
        ...
        res->m_structure->m_shards[0] = new ShardWIRS(shards, 2, res->m_structure->m_bfs[0], tagging);
        return res;
    }

    void append_buffer(MutableBuffer* buffer) {
        assert(m_shard_cnt < m_structure->m_cap);
        m_structure->m_bfs[m_shard_cnt] = new psudb::BloomFilter<skey_t>(BF_FPR, buffer->get_tombstone_count(), BF_HASH_FUNCS);
        m_structure->m_shards[m_shard_cnt] = new ShardWIRS(buffer, m_structure->m_bfs[m_shard_cnt], m_tagging);
        ++m_shard_cnt;
    }
    ...
    void append_merged_shards(Level* level) {
        ...
        m_structure->m_shards[m_shard_cnt] = new ShardWIRS(level->m_structure->m_shards, level->m_shard_cnt, m_structure->m_bfs[m_shard_cnt], m_tagging);
        ++m_shard_cnt;
    }

    ShardWIRS *get_merged_shard() {
        ShardWIRS *shards[m_shard_cnt];

        for (size_t i=0; i<m_shard_cnt; i++) {
            shards[i] = (m_structure->m_shards[i]) ? m_structure->m_shards[i] : nullptr;
        }

        return new ShardWIRS(shards, m_shard_cnt, nullptr, m_tagging);
    }

    void get_shard_weights(std::vector<double>& weights, std::vector<std::pair<ShardId, ShardWIRS *>> &shards, 
                           std::vector<ShardWIRSState*> &states, skey_t lower, skey_t upper) {

        for (size_t i=0; i<m_shard_cnt; i++) {
            if (m_structure->m_shards[i] && m_structure->m_shards[i]->get_total_weight() > 0) {
                auto state = m_structure->m_shards[i]->get_shard_sample_state(lower, upper);
    ...
    ShardWIRS* get_shard(size_t idx) {

...
START_TEST(t_buffer_init)
{
    ...
    ShardWIRS* shard = new ShardWIRS(buffer, bf, false);
...
START_TEST(t_shard_init)
{
    ...
    auto shard1 = new ShardWIRS(buffer1, bf1, false);
    auto shard2 = new ShardWIRS(buffer2, bf2, false);
    auto shard3 = new ShardWIRS(buffer3, bf3, false);

    BloomFilter<skey_t>* bf4 = new BloomFilter<skey_t>(100, BF_HASH_FUNCS);
    ShardWIRS* shards[3] = {shard1, shard2, shard3};
    auto shard4 = new ShardWIRS(shards, 3, bf4, false);
...
START_TEST(t_full_cancelation)
{
    ...
    ShardWIRS* shard = new ShardWIRS(buffer, bf1, false);
    ShardWIRS* shard_ts = new ShardWIRS(buffer_ts, bf2, false);
    ...
    ShardWIRS* shards[] = {shard, shard_ts};

    ShardWIRS* merged = new ShardWIRS(shards, 2, bf3, false);
...
START_TEST(t_weighted_sampling)
{
    ...
    for (size_t i=0; i<1000; i++) {
        auto state = shard->get_shard_sample_state(lower_key, upper_key);
        shard->get_samples(state, buf, lower_key, upper_key, k, nullptr, g_rng);
...
START_TEST(t_tombstone_check)
{
    ...
    BloomFilter<skey_t>* bf1 = new BloomFilter<skey_t>(100, BF_HASH_FUNCS);
    auto shard = new ShardWIRS(buffer, bf1, false);
...
Suite *unit_testing()
{
    Suite *unit = suite_create("ShardWIRS Unit Testing");

    TCase *create = tcase_create("extension::ShardWIRS constructor Testing");
    ...
    TCase *bounds = tcase_create("extension::ShardWIRS::get_{lower,upper}_bound Testing");
    tcase_add_test(bounds, t_get_lower_bound_index);
    tcase_set_timeout(bounds, 100);   
    suite_add_tcase(unit, bounds);


    TCase *tombstone = tcase_create("extension::ShardWIRS::tombstone cancellation Testing");
    tcase_add_test(tombstone, t_full_cancelation);
    suite_add_tcase(unit, tombstone);


    TCase *sampling = tcase_create("extension::ShardWIRS::sampling Testing");
    tcase_add_test(sampling, t_weighted_sampling);

    suite_add_tcase(unit, sampling);

    TCase *check_ts = tcase_create("extension::ShardWIRS::check_tombstone Testing");
